...
@connect()
class LeaveMessageList extends Component {
  dataContainer = [];
  ...
  _fatchData = () => {
    var param = {
      ...global.constansts.commonParam,
        "count": 10,
        "maxId": (this.state.sourceData.length == 0?0:this.state.sourceData[this.state.sourceData.length-1].messasgeId)
      }
    fetch('http://wxapi.chunchenji.com/api/services/SCMS/messageService/GetList',{
      method:'post',
       headers: {
  ...
  _renderItem = ({ item }) => {
    return ( <FlatListItem navigation={this.props.navigation} key={item.messasgeId} createdDate={item.createdDate} id={item.messasgeId}  des={item.content} img={item.wxAvatarUrl}   />);
  }
  render() {
    ...
    return (
      <ListItem  style={{ marginLeft:0, paddingLeft: 10 }} onPress={this._onPress} >
              ...
              <Body>
                ...
                <Text note>{this.props.createdDate}</Text>
              </Body>
            </ListItem>
